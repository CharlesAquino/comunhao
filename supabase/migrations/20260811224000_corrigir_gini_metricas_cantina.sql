-- Corrige o agrupamento do denominador no coeficiente de Gini.
...
begin
  ...
  if p_semanas < 4 or p_semanas > 52 then raise exception 'JANELA_INVALIDA'; end if;
  v_inicio := date_trunc('week', now()) - make_interval(weeks => p_semanas - 1);

  with
  creditos_regulares as (
    select k.usuario_id, k.quantidade, k.tipo,
           date_trunc('week', k.criado_em) as semana
    from public.kesef_ledger k
    where k.criado_em >= v_inicio and k.quantidade > 0
  ...
  saldos as (
    ...
    from public.usuarios u left join public.kesef_ledger k on k.usuario_id = u.id
    group by u.id
  ),
  saldos_ordenados as (
  ...
  tipos as (
    ...
    from public.kesef_ledger k where k.criado_em >= v_inicio group by k.tipo
  )
  select jsonb_build_object(
    ...
    'saldos', jsonb_build_object(
      ...
      'gini', coalesce((select round(sum((2 * posicao - total_membros - 1) * saldo) / nullif(max(total_membros) * max(saldo_total), 0), 4) from saldos_ordenados), 0)
    ),
