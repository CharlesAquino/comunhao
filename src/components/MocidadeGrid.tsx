...
export default function MocidadeGrid({ jovens = [], sessoesAbertas = {}, onJuntarSessao }: MocidadeGridExtendedProps) {
  return (
    <section className="community-presence" aria-labelledby="community-presence-title">
      <div className="community-presence__header">
        <div>
          <span className="community-presence__kicker">Saguão de oração</span>
          <h3 id="community-presence-title">A Mocidade</h3>
        </div>
        <div className="community-presence__status" aria-label={`${jovens.length} ${jovens.length === 1 ? 'pessoa disponível' : 'pessoas disponíveis'}`}>
          <span className="community-presence__pulse" aria-hidden="true" />
          <strong>{jovens.length}</strong>
          <span>{jovens.length === 1 ? 'disponível' : 'disponíveis'}</span>
        </div>
      </div>
      {jovens.length === 0 ? (
        <div className="premium-surface rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-4 py-5 text-center">
          <p className="text-sm font-medium text-[var(--text-secondary)]">Nenhum jovem disponível agora.</p>
          <p className="mt-1 text-xs text-[var(--text-muted)]">Quando alguém ficar disponível, aparecerá aqui.</p>
        </div>
      ) : (
        <div
          className="community-presence__rail presence-carousel scrollbar-hidden"
          role="list"
          aria-label={`${jovens.length} pessoas disponíveis agora. Deslize para ver mais.`}
        >
        {jovens.map((jovem) => {
